// JavaScript source code
export var BCQFNAV02 = `
    <div class="questform" id="bcqfnav02">
        <h2> Whose certificate you are requesting?</h2>
        <form>
            <div>
                <input type="radio" id="mechoice1" name="forme" checked />
                <label for="mechoice1"> For myself </label>
            </div>
            <div>
                <input type="radio" id="mechoice2" name="forme" />
                <label for="mechoice2"> For someone else </label>
            </div>
            <div class="button">
                <input type="button" id="nav02btn" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV01A = `
    <div class="questform" id="bcdfnav1a">
        <h2>Your details</h2>
        <form>
            <div>
                <label> Your first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Your current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Your surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of your birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of your birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of your birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV01B = `
    <div class="questform" id="bcdfnav1b">
        <h2>Details of the person you are applying for</h2>
        <form>
            <div>
                <label> Their first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Their current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Their surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of their birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of their birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV02A = `
    <div class="questform" id="bcdfnav2a">
        <h2>Details of your parents</h2>
        <form>
            <div>
                <label> Your mother\'s first and middle names: </label>
                <input type="text" id="Fname03">
            </div>
            <div>
                <label> Your mother\'s current surname: </label>
                <input type="text" id="Csname03">
            </div>
            <div>
                <label> Your mother\'s surname at birth: </label>
                <input type="text" id="Bsname03">
            </div>
            <div>
                <label> Your father\'s first and middle names: </label>
                <input type="text" id="Fname04">
            </div>
            <div>
                <label> Your father\'s current surname: </label>
                <input type="text" id="Csname04">
            </div>
            <div>
                <label> Your father\'s surname at birth: </label>
                <input type="text" id="Bsname04">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV02B = `
    <div class="questform" id="bcdfnav2b">
        <h2>Details of  parents of the person you are applying for</h2>
        <form>
            <div>
                <label> Their mother\'s first and middle names: </label>
                <input type="text" id="Fname03">
            </div>
            <div>
                <label> Their mother\'s current surname: </label>
                <input type="text" id="Csname03">
            </div>
            <div>
                <label> Their mother\'s surname at birth: </label>
                <input type="text" id="Bsname03">
            </div>
            <div>
                <label> Their father\'s first and middle names: </label>
                <input type="text" id="Fname04">
            </div>
            <div>
                <label> Their father\'s current surname: </label>
                <input type="text" id="Csname04">
            </div>
            <div>
                <label> Your father\'s surname at birth: </label>
                <input type="text" id="Bsname04">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV03A = `
    <div class="questform" id="bcdfnav3a">
        <h2> If you were born before 31.12.1945 what was your religion at birth</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - born after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV03B = `
    <div class="questform" id="bcdfnav3b">
        <h2> If they were born before 31.12.1945 what was their religion at birth</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - born after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV04A = `
    <div class="questform" id="bcdfnav4a">
        <h2> What is your relation to the person you are applying for? </h2>
        <form>
            <div> You have already indicated you are applying for yourself</div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `
export var BCDFNAV04B = `
    <div class="questform" id="bcdfnav4b">
        <h2> What is your relation to the person you are applying for ? </h2>
        <form>
            <div>
                <input type="radio" id="relachoice1" name="relation" checked />
                <label for="relachoice1"> I am their child </label>
            </div>
            <div>
                <input type="radio" id="relachoice2" name="relation" />
                <label for="relachoice2"> I am their parent </label>
            </div>
            <div>
                <input type="radio" id="relachoice3" name="relation" />
                <label for="relachoice3"> I am their granchild </label>
            </div>
            <div>
                <input type="radio" id="relachoice4" name="relation" />
                <label for="relachoice4"> I am their grandparent </label>
            </div>
            <div>
                <input type="radio" id="relachoice5" name="relation" />
                <label for="relachoice5"> I am their descenant in direct line </label>
            </div>
            <div>
                <input type="radio" id="relachoice6" name="relation" />
                <label for="relachoice6"> I am their spouse </label>
            </div>
            <div>
                <input type="radio" id="relachoice7" name="relation" />
                <label for="relachoice7"> I am their sibling </label>
            </div>
            <div>
                <input type="radio" id="relachoice8" name="relation" />
                <label for="relachoice8"> I am authorized person who can apply </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV05A = `
    <div class="questform" id="bcdfnav5a">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV05B = `
    <div class="questform" id="bcdfnav5b">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What are your first and middle names: </label>
                <input type="text" id="Fname05">
            </div>
            <div>
                <label> What is your current surname: </label>
                <input type="text" id="Csname05">
            </div>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV06A = `
    <div class="questform" id="bcdfnav6a">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Your first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Your current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Your surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of your birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of your birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of your birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Your mother\'s first and middle names: </td>
                <td id="SFname03"> </td>
            </tr>
            <tr>
                <td> Your mother\'s current surname: </td>
                <td id="SCsname03"> </td>
            </tr>
            <tr>
                <td> Your mother\'s surname at birth: </td>
                <td id="SBsname03"> </td>
            </tr>
            <tr>
                <td> Your father\'s first and middle names: </td>
                <td id="SFname04"> </td>
            </tr>
            <tr>
                <td> Your father\'s current surname: </td>
                <td id="SCsname04"> </td>
            </tr>
            <tr>
                <td>Your father\'s surname at birth:</td>
                <td id="SBsname04"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV06B = `
    <div class="questform" id="bcdfnav6b">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Their first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Their current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Their surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of their birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of their birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of their birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Their mother\'s first and middle names: </td>
                <td id="SFname03"> </td>
            </tr>
            <tr>
                <td> Their mother\'s current surname: </td>
                <td id="SCsname03"> </td>
            </tr>
            <tr>
                <td> Their mother\'s surname at birth: </td>
                <td id="SBsname03"> </td>
            </tr>
            <tr>
                <td> Their father\'s first and middle names: </td>
                <td id="SFname04"> </td>
            </tr>
            <tr>
                <td> Their father\'s current surname: </td>
                <td id="SCsname04"> </td>
            </tr>
            <tr>
                <td> Their father\'s surname at birth:</td>
                <td id="SBsname04"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td> Applicant\'s address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td> Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCQFNAV02 = `
    <div class="questform" id="mcqfnav02">
        <h2> Whose certificate you are requesting?</h2>
        <form>
            <div>
                <input type="radio" id="mechoice1" name="forme" checked />
                <label for="mechoice1"> For myself </label>
            </div>
            <div>
                <input type="radio" id="mechoice2" name="forme" />
                <label for="mechoice2"> For someone else </label>
            </div>
            <div class="button">
                <input type="button" id="nav02btn" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV01A = `
    <div class="questform" id="mcdfnav1a">
        <h2>Your details</h2>
        <form>
            <div>
                <label> Your first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Your current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Your surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of your birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of your birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of your birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV01B = `
    <div class="questform" id="mcdfnav1b">
        <h2>Details of the person you are applying for</h2>
        <form>
            <div>
                <label> Their first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Their current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Their surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of their birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of their birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV02A = `
    <div class="questform" id="mcdfnav2a">
        <h2>Details of your spouse</h2>
        <form>
            <div>
                <label> Your spouse\'s first and middle names: </label>
                <input type="text" id="Fname02">
            </div>
            <div>
                <label> Your spouse\'s current surname: </label>
                <input type="text" id="Csname02">
            </div>
            <div>
                <label> Your spouse\'s surname at birth: </label>
                <input type="text" id="Bsname02">
            </div>
            <div>
                <label>Date of your spouse\'s birth: </label>
                <input type="date" id="Dob02">
            </div>
            <div>
                <label>Place of your spouse\'s birth (town): </label>
                <input type="text" id="Pob02">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob02">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV02B = `
    <div class="questform" id="mcdfnav2b">
        <h2>Details of their spouse</h2>
        <form>
            <div>
                <label> Their spouse\'s first and middle names: </label>
                <input type="text" id="Fname02">
            </div>
            <div>
                <label> Their spouse\'s current surname: </label>
                <input type="text" id="Csname02">
            </div>
            <div>
                <label> Their spouse\'s surname at birth: </label>
                <input type="text" id="Bsname02">
            </div>
            <div>
                <label>Date of their spouse\'s birth: </label>
                <input type="date" id="Dob02">
            </div>
            <div>
                <label>Place of their spouse\'s birth (town): </label>
                <input type="text" id="Pob02">
            </div>
            <div>
                <label>Country of their spouse\'s birth</label>
                <input type="text" id="Cob02">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV03A = `
    <div class="questform" id="mcdfnav3a">
        <h2>Details of your marriage</h2>
        <form>
            <div>
                <label>Date of your marriage: </label>
                <input type="date" id="Dom01">
            </div>
            <div>
                <label>Place of your marriage (town): </label>
                <input type="text" id="Pom01">
            </div>
            <div>
                <label>Country of your marriage</label>
                <input type="text" id="Com01">
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV03B = `
    <div class="questform" id="mcdfnav3b">
        <h2>Details of their marriage</h2>
        <form>
            <div>
                <label>Date of their marriage: </label>
                <input type="date" id="Dom01">
            </div>
            <div>
                <label>Place of their marriage (town): </label>
                <input type="text" id="Pom01">
            </div>
            <div>
                <label>Country of their marriage</label>
                <input type="text" id="Com01">
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV04A = `
    <div class="questform" id="mcdfnav4a">
        <h2> If you were married before 31.12.1945 what was your religion at time of your marriage</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - married after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV04B = `
    <div class="questform" id="mcdfnav4b">
        <h2> If they were married before 31.12.1945 what was your religion at time of their marriage</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - married after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV05A = `
    <div class="questform" id="mcdfnav5a">
        <h2> What is your relation to the person you are applying for? </h2>
        <form>
            <div> You have already indicated you are applying for yourself</div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV05B = `
    <div class="questform" id="mcdfnav5b">
        <h2> What is your relation to the person you are applying for ? </h2>
        <form>
            <div>
                <input type="radio" id="relachoice1" name="relation" checked />
                <label for="relachoice1"> I am their child </label>
            </div>
            <div>
                <input type="radio" id="relachoice2" name="relation" />
                <label for="relachoice2"> I am their parent </label>
            </div>
            <div>
                <input type="radio" id="relachoice3" name="relation" />
                <label for="relachoice3"> I am their granchild </label>
            </div>
            <div>
                <input type="radio" id="relachoice4" name="relation" />
                <label for="relachoice4"> I am their grandparent </label>
            </div>
            <div>
                <input type="radio" id="relachoice5" name="relation" />
                <label for="relachoice5"> I am their descenant in direct line </label>
            </div>
            <div>
                <input type="radio" id="relachoice6" name="relation" />
                <label for="relachoice6"> I am their spouse </label>
            </div>
            <div>
                <input type="radio" id="relachoice7" name="relation" />
                <label for="relachoice7"> I am their sibling </label>
            </div>
            <div>
                <input type="radio" id="relachoice8" name="relation" />
                <label for="relachoice8"> I am authorized person who can apply </label>
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV06A = `
    <div class="questform" id="mcdfnav6a">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV06B = `
    <div class="questform" id="mcdfnav6b">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What are your first and middle names: </label>
                <input type="text" id="Fname05">
            </div>
            <div>
                <label> What is your current surname: </label>
                <input type="text" id="Csname05">
            </div>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV07A = `
    <div class="questform" id=",cdfnav7a">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Your first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Your current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Your surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of your birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of your birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of your birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Your spouse\'s first and middle names: </td>
                <td id="SFname02"> </td>
            </tr>
            <tr>
                <td> Your spouse\'s current surname: </td>
                <td id="SCsname02"> </td>
            </tr>
            <tr>
                <td> Your spouse\'s surname at birth: </td>
                <td id="SBsname02"> </td>
            </tr>
            <tr>
                <td> Date of your spouse\'s birth: </td>
                <td id="SDob02"> </td>
            </tr>
            <tr>
                <td> Place of your spouse\'s birth (town): </td>
                <td id="SPob02"> </td>
            </tr>
            <tr>
                <td> Country of your spouse\'s birth: </td>
                <td id="SCob02"> </td>
            </tr>
            <tr>
                <td> Date of your marriage: </td>
                <td id="SDom01"> </td>
            </tr>
            <tr>
                <td> Place of your marriage (town): </td>
                <td id="SPom01"> </td>
            </tr>
            <tr>
                <td> Country of your marriage: </td>
                <td id="SCom01"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df07btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV07B = `
    <div class="questform" id="mcdfnav7b">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Their first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Their current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Their surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of thier birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of their birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of their birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Their spouse\'s first and middle names: </td>
                <td id="SFname02"> </td>
            </tr>
            <tr>
                <td> Their spouse\'s current surname: </td>
                <td id="SCsname02"> </td>
            </tr>
            <tr>
                <td> Their spouse\'s surname at birth: </td>
                <td id="SBsname02"> </td>
            </tr>
            <tr>
                <td> Their of your spouse\'s birth: </td>
                <td id="SDob02"> </td>
            </tr>
            <tr>
                <td> Place of their spouse\'s birth (town): </td>
                <td id="SPob02"> </td>
            </tr>
            <tr>
                <td> Country of their spouse\'s birth: </td>
                <td id="SCob02"> </td>
            </tr>
            <tr>
                <td> Date of their marriage: </td>
                <td id="SDom01"> </td>
            </tr>
            <tr>
                <td> Place of their marriage (town): </td>
                <td id="SPom01"> </td>
            </tr>
            <tr>
                <td> Country of their marriage: </td>
                <td id="SCom01"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df07btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV01A = `
    <div class="questform" id="dcdfnav1a">
        <h2>Details of the person you are applying for</h2>
        <form>
            <div>
                <label> Their first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Their current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Their surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of their birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of their birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV02A = `
    <div class="questform" id="dcdfnav2a">
        <h2>Details of their death</h2>
        <form>
            <div>
                <label>Date of their death: </label>
                <input type="date" id="Dod01">
            </div>
            <div>
                <label>Place of their death (town): </label>
                <input type="text" id="Pod01">
            </div>
            <div>
                <label>Country of their death</label>
                <input type="text" id="Comd1">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV03A = `
    <div class="questform" id="dcdfnav3a">
        <h2> If they died before 31.12.1945 what was your religion at time of their death</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - died after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV04A = `
    <div class="questform" id="dcdfnav4a">
        <h2> What is your relation to the person you are applying for ? </h2>
        <form>
            <div>
                <input type="radio" id="relachoice1" name="relation" checked />
                <label for="relachoice1"> I am their child </label>
            </div>
            <div>
                <input type="radio" id="relachoice2" name="relation" />
                <label for="relachoice2"> I am their parent </label>
            </div>
            <div>
                <input type="radio" id="relachoice3" name="relation" />
                <label for="relachoice3"> I am their granchild </label>
            </div>
            <div>
                <input type="radio" id="relachoice4" name="relation" />
                <label for="relachoice4"> I am their grandparent </label>
            </div>
            <div>
                <input type="radio" id="relachoice5" name="relation" />
                <label for="relachoice5"> I am their descenant in direct line </label>
            </div>
            <div>
                <input type="radio" id="relachoice6" name="relation" />
                <label for="relachoice6"> I am their spouse </label>
            </div>
            <div>
                <input type="radio" id="relachoice7" name="relation" />
                <label for="relachoice7"> I am their sibling </label>
            </div>
            <div>
                <input type="radio" id="relachoice8" name="relation" />
                <label for="relachoice8"> I am authorized person who can apply </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `


export var DCDFNAV05A = `
    <div class="questform" id="dcdfnav5a">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What are your first and middle names: </label>
                <input type="text" id="Fname05">
            </div>
            <div>
                <label> What is your current surname: </label>
                <input type="text" id="Csname05">
            </div>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV06A = `
    <div class="questform" id="dcdfnav6a">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Their first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Their current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Their surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of their birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of their birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of their birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Date of their death: </td>
                <td id="SDod01"> </td>
            </tr>
            <tr>
                <td> Place of their death (town): </td>
                <td id="SPod01"> </td>
            </tr>
            <tr>
                <td> Country of their death: </td>
                <td id="SCod01"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `    // JavaScript source code
export var BCQFNAV02 = `
    <div class="questform" id="bcqfnav02">
        <h2> Whose certificate you are requesting?</h2>
        <form>
            <div>
                <input type="radio" id="mechoice1" name="forme" checked />
                <label for="mechoice1"> For myself </label>
            </div>
            <div>
                <input type="radio" id="mechoice2" name="forme" />
                <label for="mechoice2"> For someone else </label>
            </div>
            <div class="button">
                <input type="button" id="nav02btn" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV01A = `
    <div class="questform" id="bcdfnav1a">
        <h2>Your details</h2>
        <form>
            <div>
                <label> Your first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Your current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Your surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of your birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of your birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of your birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV01B = `
    <div class="questform" id="bcdfnav1b">
        <h2>Details of the person you are applying for</h2>
        <form>
            <div>
                <label> Their first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Their current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Their surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of their birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of their birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV02A = `
    <div class="questform" id="bcdfnav2a">
        <h2>Details of your parents</h2>
        <form>
            <div>
                <label> Your mother\'s first and middle names: </label>
                <input type="text" id="Fname03">
            </div>
            <div>
                <label> Your mother\'s current surname: </label>
                <input type="text" id="Csname03">
            </div>
            <div>
                <label> Your mother\'s surname at birth: </label>
                <input type="text" id="Bsname03">
            </div>
            <div>
                <label> Your father\'s first and middle names: </label>
                <input type="text" id="Fname04">
            </div>
            <div>
                <label> Your father\'s current surname: </label>
                <input type="text" id="Csname04">
            </div>
            <div>
                <label> Your father\'s surname at birth: </label>
                <input type="text" id="Bsname04">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV02B = `
    <div class="questform" id="bcdfnav2b">
        <h2>Details of  parents of the person you are applying for</h2>
        <form>
            <div>
                <label> Their mother\'s first and middle names: </label>
                <input type="text" id="Fname03">
            </div>
            <div>
                <label> Their mother\'s current surname: </label>
                <input type="text" id="Csname03">
            </div>
            <div>
                <label> Their mother\'s surname at birth: </label>
                <input type="text" id="Bsname03">
            </div>
            <div>
                <label> Their father\'s first and middle names: </label>
                <input type="text" id="Fname04">
            </div>
            <div>
                <label> Their father\'s current surname: </label>
                <input type="text" id="Csname04">
            </div>
            <div>
                <label> Your father\'s surname at birth: </label>
                <input type="text" id="Bsname04">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV03A = `
    <div class="questform" id="bcdfnav3a">
        <h2> If you were born before 31.12.1945 what was your religion at birth</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - born after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV03B = `
    <div class="questform" id="bcdfnav3b">
        <h2> If they were born before 31.12.1945 what was their religion at birth</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - born after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV04A = `
    <div class="questform" id="bcdfnav4a">
        <h2> What is your relation to the person you are applying for? </h2>
        <form>
            <div> You have already indicated you are applying for yourself</div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `
export var BCDFNAV04B = `
    <div class="questform" id="bcdfnav4b">
        <h2> What is your relation to the person you are applying for ? </h2>
        <form>
            <div>
                <input type="radio" id="relachoice1" name="relation" checked />
                <label for="relachoice1"> I am their child </label>
            </div>
            <div>
                <input type="radio" id="relachoice2" name="relation" />
                <label for="relachoice2"> I am their parent </label>
            </div>
            <div>
                <input type="radio" id="relachoice3" name="relation" />
                <label for="relachoice3"> I am their granchild </label>
            </div>
            <div>
                <input type="radio" id="relachoice4" name="relation" />
                <label for="relachoice4"> I am their grandparent </label>
            </div>
            <div>
                <input type="radio" id="relachoice5" name="relation" />
                <label for="relachoice5"> I am their descenant in direct line </label>
            </div>
            <div>
                <input type="radio" id="relachoice6" name="relation" />
                <label for="relachoice6"> I am their spouse </label>
            </div>
            <div>
                <input type="radio" id="relachoice7" name="relation" />
                <label for="relachoice7"> I am their sibling </label>
            </div>
            <div>
                <input type="radio" id="relachoice8" name="relation" />
                <label for="relachoice8"> I am authorized person who can apply </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV05A = `
    <div class="questform" id="bcdfnav5a">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV05B = `
    <div class="questform" id="bcdfnav5b">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What are your first and middle names: </label>
                <input type="text" id="Fname05">
            </div>
            <div>
                <label> What is your current surname: </label>
                <input type="text" id="Csname05">
            </div>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV06A = `
    <div class="questform" id="bcdfnav6a">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Your first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Your current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Your surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of your birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of your birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of your birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Your mother\'s first and middle names: </td>
                <td id="SFname03"> </td>
            </tr>
            <tr>
                <td> Your mother\'s current surname: </td>
                <td id="SCsname03"> </td>
            </tr>
            <tr>
                <td> Your mother\'s surname at birth: </td>
                <td id="SBsname03"> </td>
            </tr>
            <tr>
                <td> Your father\'s first and middle names: </td>
                <td id="SFname04"> </td>
            </tr>
            <tr>
                <td> Your father\'s current surname: </td>
                <td id="SCsname04"> </td>
            </tr>
            <tr>
                <td>Your father\'s surname at birth:</td>
                <td id="SBsname04"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var BCDFNAV06B = `
    <div class="questform" id="bcdfnav6b">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Their first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Their current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Their surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of their birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of their birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of their birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Their mother\'s first and middle names: </td>
                <td id="SFname03"> </td>
            </tr>
            <tr>
                <td> Their mother\'s current surname: </td>
                <td id="SCsname03"> </td>
            </tr>
            <tr>
                <td> Their mother\'s surname at birth: </td>
                <td id="SBsname03"> </td>
            </tr>
            <tr>
                <td> Their father\'s first and middle names: </td>
                <td id="SFname04"> </td>
            </tr>
            <tr>
                <td> Their father\'s current surname: </td>
                <td id="SCsname04"> </td>
            </tr>
            <tr>
                <td> Their father\'s surname at birth:</td>
                <td id="SBsname04"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td> Applicant\'s address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td> Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCQFNAV02 = `
    <div class="questform" id="mcqfnav02">
        <h2> Whose certificate you are requesting?</h2>
        <form>
            <div>
                <input type="radio" id="mechoice1" name="forme" checked />
                <label for="mechoice1"> For myself </label>
            </div>
            <div>
                <input type="radio" id="mechoice2" name="forme" />
                <label for="mechoice2"> For someone else </label>
            </div>
            <div class="button">
                <input type="button" id="nav02btn" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV01A = `
    <div class="questform" id="mcdfnav1a">
        <h2>Your details</h2>
        <form>
            <div>
                <label> Your first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Your current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Your surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of your birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of your birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of your birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV01B = `
    <div class="questform" id="mcdfnav1b">
        <h2>Details of the person you are applying for</h2>
        <form>
            <div>
                <label> Their first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Their current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Their surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of their birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of their birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV02A = `
    <div class="questform" id="mcdfnav2a">
        <h2>Details of your spouse</h2>
        <form>
            <div>
                <label> Your spouse\'s first and middle names: </label>
                <input type="text" id="Fname02">
            </div>
            <div>
                <label> Your spouse\'s current surname: </label>
                <input type="text" id="Csname02">
            </div>
            <div>
                <label> Your spouse\'s surname at birth: </label>
                <input type="text" id="Bsname02">
            </div>
            <div>
                <label>Date of your spouse\'s birth: </label>
                <input type="date" id="Dob02">
            </div>
            <div>
                <label>Place of your spouse\'s birth (town): </label>
                <input type="text" id="Pob02">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob02">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV02B = `
    <div class="questform" id="mcdfnav2b">
        <h2>Details of their spouse</h2>
        <form>
            <div>
                <label> Their spouse\'s first and middle names: </label>
                <input type="text" id="Fname02">
            </div>
            <div>
                <label> Their spouse\'s current surname: </label>
                <input type="text" id="Csname02">
            </div>
            <div>
                <label> Their spouse\'s surname at birth: </label>
                <input type="text" id="Bsname02">
            </div>
            <div>
                <label>Date of their spouse\'s birth: </label>
                <input type="date" id="Dob02">
            </div>
            <div>
                <label>Place of their spouse\'s birth (town): </label>
                <input type="text" id="Pob02">
            </div>
            <div>
                <label>Country of their spouse\'s birth</label>
                <input type="text" id="Cob02">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV03A = `
    <div class="questform" id="mcdfnav3a">
        <h2>Details of your marriage</h2>
        <form>
            <div>
                <label>Date of your marriage: </label>
                <input type="date" id="Dom01">
            </div>
            <div>
                <label>Place of your marriage (town): </label>
                <input type="text" id="Pom01">
            </div>
            <div>
                <label>Country of your marriage</label>
                <input type="text" id="Com01">
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV03B = `
    <div class="questform" id="mcdfnav3b">
        <h2>Details of their marriage</h2>
        <form>
            <div>
                <label>Date of their marriage: </label>
                <input type="date" id="Dom01">
            </div>
            <div>
                <label>Place of their marriage (town): </label>
                <input type="text" id="Pom01">
            </div>
            <div>
                <label>Country of their marriage</label>
                <input type="text" id="Com01">
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV04A = `
    <div class="questform" id="mcdfnav4a">
        <h2> If you were married before 31.12.1945 what was your religion at time of your marriage</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - married after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV04B = `
    <div class="questform" id="mcdfnav4b">
        <h2> If they were married before 31.12.1945 what was your religion at time of their marriage</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - married after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV05A = `
    <div class="questform" id="mcdfnav5a">
        <h2> What is your relation to the person you are applying for? </h2>
        <form>
            <div> You have already indicated you are applying for yourself</div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV05B = `
    <div class="questform" id="mcdfnav5b">
        <h2> What is your relation to the person you are applying for ? </h2>
        <form>
            <div>
                <input type="radio" id="relachoice1" name="relation" checked />
                <label for="relachoice1"> I am their child </label>
            </div>
            <div>
                <input type="radio" id="relachoice2" name="relation" />
                <label for="relachoice2"> I am their parent </label>
            </div>
            <div>
                <input type="radio" id="relachoice3" name="relation" />
                <label for="relachoice3"> I am their granchild </label>
            </div>
            <div>
                <input type="radio" id="relachoice4" name="relation" />
                <label for="relachoice4"> I am their grandparent </label>
            </div>
            <div>
                <input type="radio" id="relachoice5" name="relation" />
                <label for="relachoice5"> I am their descenant in direct line </label>
            </div>
            <div>
                <input type="radio" id="relachoice6" name="relation" />
                <label for="relachoice6"> I am their spouse </label>
            </div>
            <div>
                <input type="radio" id="relachoice7" name="relation" />
                <label for="relachoice7"> I am their sibling </label>
            </div>
            <div>
                <input type="radio" id="relachoice8" name="relation" />
                <label for="relachoice8"> I am authorized person who can apply </label>
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV06A = `
    <div class="questform" id="mcdfnav6a">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV06B = `
    <div class="questform" id="mcdfnav6b">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What are your first and middle names: </label>
                <input type="text" id="Fname05">
            </div>
            <div>
                <label> What is your current surname: </label>
                <input type="text" id="Csname05">
            </div>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV07A = `
    <div class="questform" id=",cdfnav7a">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Your first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Your current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Your surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of your birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of your birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of your birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Your spouse\'s first and middle names: </td>
                <td id="SFname02"> </td>
            </tr>
            <tr>
                <td> Your spouse\'s current surname: </td>
                <td id="SCsname02"> </td>
            </tr>
            <tr>
                <td> Your spouse\'s surname at birth: </td>
                <td id="SBsname02"> </td>
            </tr>
            <tr>
                <td> Date of your spouse\'s birth: </td>
                <td id="SDob02"> </td>
            </tr>
            <tr>
                <td> Place of your spouse\'s birth (town): </td>
                <td id="SPob02"> </td>
            </tr>
            <tr>
                <td> Country of your spouse\'s birth: </td>
                <td id="SCob02"> </td>
            </tr>
            <tr>
                <td> Date of your marriage: </td>
                <td id="SDom01"> </td>
            </tr>
            <tr>
                <td> Place of your marriage (town): </td>
                <td id="SPom01"> </td>
            </tr>
            <tr>
                <td> Country of your marriage: </td>
                <td id="SCom01"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df07btn1" value="Next" />
            </div>
        </form>
    </div> `

export var MCDFNAV07B = `
    <div class="questform" id="mcdfnav7b">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Their first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Their current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Their surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of thier birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of their birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of their birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Their spouse\'s first and middle names: </td>
                <td id="SFname02"> </td>
            </tr>
            <tr>
                <td> Their spouse\'s current surname: </td>
                <td id="SCsname02"> </td>
            </tr>
            <tr>
                <td> Their spouse\'s surname at birth: </td>
                <td id="SBsname02"> </td>
            </tr>
            <tr>
                <td> Their of your spouse\'s birth: </td>
                <td id="SDob02"> </td>
            </tr>
            <tr>
                <td> Place of their spouse\'s birth (town): </td>
                <td id="SPob02"> </td>
            </tr>
            <tr>
                <td> Country of their spouse\'s birth: </td>
                <td id="SCob02"> </td>
            </tr>
            <tr>
                <td> Date of their marriage: </td>
                <td id="SDom01"> </td>
            </tr>
            <tr>
                <td> Place of their marriage (town): </td>
                <td id="SPom01"> </td>
            </tr>
            <tr>
                <td> Country of their marriage: </td>
                <td id="SCom01"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df07btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV01A = `
    <div class="questform" id="dcdfnav1a">
        <h2>Details of the person you are applying for</h2>
        <form>
            <div>
                <label> Their first and middle names: </label>
                <input type="text" id="Fname01">
            </div>
            <div>
                <label> Their current surname: </label>
                <input type="text" id="Csname01">
            </div>
            <div>
                <label> Their surname at birth: </label>
                <input type="text" id="Bsname01">
            </div>
            <div>
                <label>Date of their birth: </label>
                <input type="date" id="Dob01">
            </div>
            <div>
                <label>Place of their birth (town): </label>
                <input type="text" id="Pob01">
            </div>
            <div>
                <label>Country of their birth</label>
                <input type="text" id="Cob01">
            </div>
            <div class="button">
                <input type="button" id="df01btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV02A = `
    <div class="questform" id="dcdfnav2a">
        <h2>Details of their death</h2>
        <form>
            <div>
                <label>Date of their death: </label>
                <input type="date" id="Dod01">
            </div>
            <div>
                <label>Place of their death (town): </label>
                <input type="text" id="Pod01">
            </div>
            <div>
                <label>Country of their death</label>
                <input type="text" id="Comd1">
            </div>
            <div class="button">
                <input type="button" id="df02btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV03A = `
    <div class="questform" id="dcdfnav3a">
        <h2> If they died before 31.12.1945 what was your religion at time of their death</h2>
        <form>
            <div>
                <input type="radio" id="relchoice1" name="religion" checked />
                <label for="relchoice1"> Not applicable - died after 31.12.1945 </label>
            </div>
            <div>
                <input type="radio" id="relchoice2" name="religion" />
                <label for="relchoice2"> Catholic </label>
            </div>
            <div>
                <input type="radio" id="relchoice3" name="religion" />
                <label for="relchoice3"> Protestant </label>
            </div>
            <div>
                <input type="radio" id="relchoice4" name="religion" />
                <label for="relchoice4"> Jewish </label>
            </div>
            <div class="button">
                <input type="button" id="df03btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV04A = `
    <div class="questform" id="dcdfnav4a">
        <h2> What is your relation to the person you are applying for ? </h2>
        <form>
            <div>
                <input type="radio" id="relachoice1" name="relation" checked />
                <label for="relachoice1"> I am their child </label>
            </div>
            <div>
                <input type="radio" id="relachoice2" name="relation" />
                <label for="relachoice2"> I am their parent </label>
            </div>
            <div>
                <input type="radio" id="relachoice3" name="relation" />
                <label for="relachoice3"> I am their granchild </label>
            </div>
            <div>
                <input type="radio" id="relachoice4" name="relation" />
                <label for="relachoice4"> I am their grandparent </label>
            </div>
            <div>
                <input type="radio" id="relachoice5" name="relation" />
                <label for="relachoice5"> I am their descenant in direct line </label>
            </div>
            <div>
                <input type="radio" id="relachoice6" name="relation" />
                <label for="relachoice6"> I am their spouse </label>
            </div>
            <div>
                <input type="radio" id="relachoice7" name="relation" />
                <label for="relachoice7"> I am their sibling </label>
            </div>
            <div>
                <input type="radio" id="relachoice8" name="relation" />
                <label for="relachoice8"> I am authorized person who can apply </label>
            </div>
            <div class="button">
                <input type="button" id="df04btn1" value="Next" />
            </div>
        </form>
    </div> `


export var DCDFNAV05A = `
    <div class="questform" id="dcdfnav5a">
        <h2> Further details about you</h2>
        <form>
            <div>
                <label> What are your first and middle names: </label>
                <input type="text" id="Fname05">
            </div>
            <div>
                <label> What is your current surname: </label>
                <input type="text" id="Csname05">
            </div>
            <div>
                <label> What is your postal address: </label>
                <input type="text" id="Padd01">
            </div>
            <div class="button">
                <input type="button" id="df05btn1" value="Next" />
            </div>
        </form>
    </div> `

export var DCDFNAV06A = `
    <div class="questform" id="dcdfnav6a">
        <h2> Summary of details </h2>
        <table>
            <tr>
                <td> Their first and middle names: </td>
                <td id="SFname01"> </td>
            </tr>
            <tr>
                <td> Their current surname: </td>
                <td id="SCsname01"> </td>
            </tr>
            <tr>
                <td> Their surname at birth: </td>
                <td id="SBsname01"> </td>
            </tr>
            <tr>
                <td> Date of their birth: </td>
                <td id="SDob01"> </td>
            </tr>
            <tr>
                <td> Place of their birth (town): </td>
                <td id="SPob01"> </td>
            </tr>
            <tr>
                <td> Country of their birth: </td>
                <td id="SCob01"> </td>
            </tr>
            <tr>
                <td> Date of their death: </td>
                <td id="SDod01"> </td>
            </tr>
            <tr>
                <td> Place of their death (town): </td>
                <td id="SPod01"> </td>
            </tr>
            <tr>
                <td> Country of their death: </td>
                <td id="SCod01"> </td>
            </tr>
            <tr>
                <td>Religion</td>
                <td id="SReli"> </td>
            </tr>
            <tr>
                <td>Relation</td>
                <td id="SRela"> </td>
            </tr>
            <tr>
                <td>Your address</td>
                <td id="SPadd01"> </td>
            </tr>
            <tr>
                <td> Applicant\'s first and middle names</td>
                <td id="SFname05"> </td>
            </tr>
            <tr>
                <td>Applicant\'s current surname </td>
                <td id="SCsname05"> </td>
            </tr>
        </table>
        <form>
            <div class="button">
                <input type="button" id="df06btn1" value="Next" />
            </div>
        </form>
    </div> `